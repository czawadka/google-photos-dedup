...
gpdedup produces a REPORT ONLY: it cannot delete photos or fix album membership
(no Google Photos library API), and the keep-decision depends on the live "backed
up / not consuming storage" status visible only in the Photos UI. The report
surfaces each confirmed pair (dates + sizes + per-copy deep links); you confirm
the keeper and act in the Photos web UI. See README.md.

Usage:
    export DRIVE_TOKEN="ya29...."
    uv run gpdedup --max-parts 0
    uv run gpdedup --offline      # rebuild from cache, no token
"""
...
from gpdedup.central_dir import read_part_entries
from gpdedup.drive import (
    TAKEOUT_QUERY, auth_header, list_files, resolve_token, with_trashed_filter,
)
from gpdedup.dating import capture_dates, real_dup_pairs
from gpdedup.grouping import (
    group_candidates, is_media_file, is_truncated_name,
)
from gpdedup.http_range import RangeNotSupported
from gpdedup.report import token_search_url, write_table_html
from gpdedup.sidecar import resolve_urls
...
def index_parts(args, conn):
    ...
    if args.offline:
        ...
        parts = [f for f in list_files(token, with_trashed_filter(args.query))
...
def main() -> int:
    ap = argparse.ArgumentParser(prog="gpdedup", description=__doc__,
    ...
    ap.add_argument("--query", default=TAKEOUT_QUERY,
                    help="Drive files.list query to pick the Takeout parts. STRONGLY "
                         "RECOMMENDED to scope to one export's name prefix, e.g. "
                         "\"name contains 'takeout-20260611T191302Z-3-'\" — the default "
                         "matches every 'takeout' file and mixes multiple exports. "
                         "trashed = false is auto-added.")
    ap.add_argument("--max-parts", type=int, default=0, help="parts to read (0 = all)")
    ap.add_argument("--report", default="delete_table.html")
